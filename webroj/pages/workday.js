...
import { FaExchangeAlt, FaLongArrowAltRight, FaTachometerAlt, FaUsers } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "workday");
    ...
    const steps = [
        {
          ...
          description: 'Develop a strategic plan for effective Workday integration across your organization.',
          icon: <FaBullseye className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Change Management',
          description: 'Implement change management practices to ensure smooth transitions during adoption.',
          icon: <FaExchangeAlt className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Performance ',
          description: 'Enhance system performance to maximize the benefits of Workday’s features.',
          icon: <FaTachometerAlt className="text-4xl text-pink-600" />
        },
       
        {
          id: 4,
          title: 'User Adoption',
          description: 'Foster user adoption through tailored training and support materials.',
    ...
    return (
        <div className="about">
           <Head>
      <title>Workday - Webroj Software Company</title>
      <meta name="description" content="Discover how Workday can enhance your organization's workforce management and streamline HR processes with innovative technologies." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Workday</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Workday <span className='text-[#fc005f]'>Solutions</span>
  </h2>
