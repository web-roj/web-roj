...
import { FaCloudUploadAlt, FaDollarSign, FaLock, FaLongArrowAltRight, FaServer } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "aws");
    ...
    const steps = [
        {
          ...
          description: 'Seamlessly migrate your applications and data to AWS for enhanced scalability and performance.',
          icon: <FaCloudUploadAlt className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Infrastructure ',
          description: 'Manage your AWS infrastructure efficiently with our tailored monitoring and optimization solutions.',
          icon: <FaServer className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Data Security',
          description: 'Implement robust security measures to protect your data in the cloud with AWS security services.',
          icon: <FaLock className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Cost Optimization',
          description: 'Optimize your AWS spending with our analysis tools and strategies for cost efficiency.',
    ...
    return (
        <div className="about">
           <Head>
      <title>Amazon Web Services (AWS) - Webroj Software Company</title>
      <meta name="description" content="Leverage the power of Amazon Web Services (AWS) for scalable cloud solutions, data storage, and innovative application development. Transform your business with our expert AWS consulting." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Amazon Web Services</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Amazon Web <span className='text-[#fc005f]'>Services</span>
  </h2>
