...
import { FaChartBar, FaCog, FaDatabase, FaLongArrowAltRight } from 'react-icons/fa';
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "data-solutions");
...
const steps = [
    {
      ...
      description: 'Efficiently gather data from various sources to enhance analysis and insights.',
      icon: <FaDatabase className="text-4xl text-pink-600" />
    },
    {
      id: 2,
      title: 'Data Processing',
      description: 'Process and clean data to ensure accuracy and relevance for decision-making.',
      icon: <FaCog className="text-4xl text-pink-600" />
    },
    {
      id: 3,
      title: 'Data Analytics',
      description: 'Analyze data to uncover trends, patterns, and actionable insights.',
      icon: <FaChartLine className="text-4xl text-pink-600" />
    },
    {
      id: 4,
      title: 'Data Visualization',
      description: 'Transform complex data sets into visual representations for easier understanding.',
    ...
    return (
        <div className="about">
          <Head>
      <title>Data Solutions - Webroj Software Company</title>
      <meta name="description" content="Unlock the power of your data with our tailored data solutions that enhance decision-making and drive business growth." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Data Solutions</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Data <span className='text-[#fc005f]'>Solutions</span>
  </h2>
