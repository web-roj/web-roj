...
import { FaDatabase, FaFileContract, FaLongArrowAltRight, FaPhoneAlt, FaUsers } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "health-care");
    ...
    const steps = [
        {
          id: 1,
          title: 'Telemedicine',
          description: 'Offering remote consultations to enhance patient access and improve healthcare delivery.',
          icon: <FaPhoneAlt className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: ' Data Management',
          description: 'Streamlining patient data management to ensure accuracy and compliance with regulations.',
          icon: <FaDatabase className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Patient Engagement',
          description: 'Empowering patients with tools and resources to actively participate in their own healthcare.',
        ...
        
        {
          id: 4,
          title: 'Compliance',
          description: 'Ensuring adherence to healthcare regulations and standards to protect patient privacy and data.',
    ...
    return (
        <div className="about">
           <Head>
      <title>Healthcare - Webroj Software Company</title>
      <meta name="description" content="Explore our innovative healthcare services designed to enhance patient care, streamline operations, and improve health outcomes. Partner with us for advanced healthcare technology." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Health Care</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Healthcare <span className='text-[#fc005f]'>Solutions</span>
  </h2>
