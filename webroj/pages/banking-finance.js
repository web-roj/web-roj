...
import { FaHandshake, FaLongArrowAltRight, FaRegMoneyBillAlt, FaShieldAlt, FaWallet } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "banking-finance");
    ...
    const steps = [
        {
          ...
          description: 'Manage your accounts effortlessly with our user-friendly digital tools and services.',
          icon: <FaWallet className="text-4xl text-pink-600" />
        },
       
        {
          id: 2,
          title: 'Loan Services',
          description: 'Access personalized loan options tailored to meet your financial needs and goals.',
          icon: <FaHandshake className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Financial Planning',
          description: 'Work with financial advisors to create a solid plan for your future financial stability.',
          icon: <FaRegMoneyBillAlt className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Fraud Protection',
          description: 'Stay protected with advanced security measures to safeguard your finances from fraud.',
    ...
    return (
        <div className="about">
          <Head>
      <title>Banking and Finance - Webroj Software Company</title>
      <meta name="description" content="Explore our innovative banking and finance solutions designed to enhance financial services, streamline operations, and improve customer satisfaction." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Banking & Finance</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Banking & Finance <span className='text-[#fc005f]'>Solutions</span>
  </h2>
