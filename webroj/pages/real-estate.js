...
import { FaChartBar, FaHome, FaLongArrowAltRight, FaUsers, FaVideo } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "real-estate");
    ...
    const steps = [
        {
          ...
          description: 'Showcase properties with high-quality images and detailed descriptions to attract potential buyers.',
          icon: <FaHome className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Market Analysis',
          description: 'Conduct market research to determine property values and trends in the real estate market.',
          icon: <FaChartBar className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Consultation',
          description: 'Provide personalized consultations to help clients understand their needs and preferences.',
          icon: <FaUsers className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Virtual Tours',
          description: 'Offer virtual tours to give clients a comprehensive view of properties from the comfort of their homes.',
          icon: <FaVideo className="text-4xl text-pink-600" />
        },
    ...
    return (
        <div className="about">
           <Head>
      <title>Real Estate - Webroj Software Company</title>
      <meta name="description" content="Discover our comprehensive real estate services designed to help you buy, sell, and manage properties effectively. Partner with us for expert guidance in the real estate market." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Real Estate</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Real Estate <span className='text-[#fc005f]'>Solutions</span>
  </h2>
