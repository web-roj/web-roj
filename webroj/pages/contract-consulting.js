...
import { FaChartPie, FaClipboardList, FaLongArrowAltRight, FaRegLightbulb, FaTools } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "contract-consulting");
    ...
    const steps = [
        {
          ...
          description: 'Conduct thorough assessments to understand client needs and objectives for effective solutions.',
          icon: <FaClipboardList className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Strategic Planning',
          description: 'Develop strategic plans tailored to meet specific client goals and drive success.',
          icon: <FaRegLightbulb className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Implementation Support',
          description: 'Provide ongoing support during the implementation phase to ensure smooth execution.',
          icon: <FaTools className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Performance Evaluation',
          description: 'Assess the outcomes of consulting efforts and identify areas for improvement.',
    ...
    return (
        <div className="about">
          <Head>
      <title>Contract Consulting - Webroj Software Company</title>
      <meta name="description" content="Get expert guidance and support for your contract needs. Our consulting services ensure compliance, risk management, and streamlined processes." />
    ...
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        <Link href="/" className="block text-lg font-bold sm:text-2xl lg:text-2xl whitespace-nowrap">
          Home/Contract Consulting
        </Link>
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Contract Consulting</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Contract <span className='text-[#fc005f]'>Consulting</span>
  </h2>
