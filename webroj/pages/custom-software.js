...
import { FaCheckCircle, FaCode, FaLongArrowAltRight, FaPencilRuler } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category ==="custom-software");
    ...
    const steps = [
        {
          ...
          description: 'We gather and analyze the client’s specific requirements to ensure we develop a tailored software solution.',
          icon: <FaSearch className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'System Design',
          description: 'We design the system architecture, databases, and interfaces to ensure scalability and efficiency.',
          icon: <FaPencilRuler className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Development',
          description: 'Our development team writes clean, efficient code to build the software solution tailored to the client’s needs.',
          icon: <FaCode className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Testing & QA',
          description: 'We rigorously test the software to ensure there are no bugs or performance issues before delivery.',
          icon: <FaCheckCircle className="text-4xl text-pink-600" />
        },
    ...
    return (
        <div className="about">
          <Head>
      <title>Custom Software Development - Webroj Software Company</title>
      <meta name="description" content="Explore our custom software development services tailored to your unique business needs. We create scalable and efficient solutions to drive your success." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Custom Software</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Custom Software <span className='text-[#fc005f]'>Development</span>
  </h2>
