...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "sap");
    ...
    const steps = [
        {
          ...
          description: 'Comprehensive SAP implementation services tailored to meet your business needs, ensuring seamless integration with existing systems.',
       ...
       
        {
          id: 2,
          title: '  Maintenance',
          description: 'Ongoing support and maintenance services to ensure your SAP systems are optimized and functioning effectively.',
          icon: <FaHeadset className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Migration Services',
          description: 'Expert assistance with migrating to the latest SAP versions or transitioning from legacy systems to SAP.',
          icon: <FaCloudUploadAlt className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Analytics',
          description: 'Utilizing SAP analytics tools to derive actionable insights and enhance decision-making across your organization.',
    ...
    return (
        <div className="about">
           <Head>
      <title>SAP Solutions - Webroj Software Company</title>
      <meta name="description" content="Explore our SAP solutions tailored to enhance business processes, streamline operations, and drive digital transformation." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">SAP</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    SAP <span className='text-[#fc005f]'>Solutions</span>
  </h2>
