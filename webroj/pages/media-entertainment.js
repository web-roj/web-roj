...
import { FaBullhorn, FaFilm, FaLongArrowAltRight, FaTv } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "media-entertainment");
    ...
    const steps = [
        {
          ...
          description: 'Develop engaging content that resonates with audiences across various platforms and formats.',
          icon: <FaFilm className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Streaming Services',
          description: 'Provide reliable streaming solutions for movies, music, and live events to enhance user experience.',
          icon: <FaTv className="text-4xl text-pink-600" />
        },
       
        {
          id: 3,
          title: 'Analytics and Insights',
          description: 'Utilize data analytics to track audience behavior and refine content strategies for better engagement.',
          icon: <FaChartLine className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Advertising ',
          description: 'Offer targeted advertising solutions to maximize reach and revenue for media platforms.',
    ...
    return (
        <div className="about">
          <Head>
      <title>Media and Entertainment - Webroj Software Company</title>
      <meta name="description" content="Discover our innovative media and entertainment solutions that enhance audience engagement and deliver captivating content experiences." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Media & Entertainment</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Media & <span className='text-[#fc005f]'>Entertainment</span>
  </h2>
