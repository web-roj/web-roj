...
import { FaBoxes, FaCreditCard, FaHeadset, FaLongArrowAltRight, FaShoppingCart } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "ecommerce-solutions");
    ...
    const steps = [
        {
          id: 1,
          title: 'User-Friendly ',
          description: 'Our eCommerce solutions provide a seamless shopping experience with intuitive design and navigation.',
          icon: <FaShoppingCart className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Payment Integration',
          description: 'Secure payment gateways to facilitate smooth transactions, ensuring customer trust and satisfaction.',
          icon: <FaCreditCard className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: ' Management',
          description: 'Efficient tools for managing inventory, tracking stock levels, and forecasting demand.',
          icon: <FaBoxes className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Customer Support',
          description: 'Dedicated customer support solutions, including chatbots and live agents, to assist shoppers.',
    ...
    return (
        <div className="about">
           <Head>
      <title>E-commerce Solutions - Webroj Software Company</title>
      <meta name="description" content="Explore our comprehensive e-commerce solutions that empower businesses to create seamless online shopping experiences and drive sales." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Ecommerce Solutions</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    eCommerce <span className='text-[#fc005f]'>Solutions</span>
  </h2>
