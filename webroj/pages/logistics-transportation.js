...
import { FaLongArrowAltRight, FaMapMarkedAlt, FaShippingFast, FaTruck } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "logistics-transportation");
    ...
    const steps = [
        {
          id: 1,
          title: 'Supply Chain ',
          description: 'Optimize supply chain operations for efficient movement of goods from origin to destination.',
          icon: <FaTruck className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Freight Forwarding',
          description: 'Coordinate the logistics of transporting goods via air, sea, or land to ensure timely delivery.',
          icon: <FaShippingFast className="text-4xl text-pink-600" />
        },
       
        {
          id: 3,
          title: 'Real-Time Tracking',
          description: 'Implement real-time tracking solutions for shipments to enhance transparency and reliability.',
          icon: <FaMapMarkedAlt className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Transportation ',
          description: 'Utilize data analytics to optimize routes and reduce transportation costs for better efficiency.',
    ...
    return (
        <div className="about">
          <Head>
      <title>Logistics and Transportation - Webroj Software Company</title>
      <meta name="description" content="Explore our logistics and transportation solutions that optimize supply chain efficiency and streamline operations for seamless delivery." />
    ...
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        <Link href="/" className="block text-lg font-bold sm:text-2xl lg:text-2xl whitespace-nowrap">
          Home/Logistics & Transportation
        </Link>
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Logistics & Transportation</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Logistics & <span className='text-[#fc005f]'>Transportation</span>
  </h2>
