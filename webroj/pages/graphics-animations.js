...
import { FaChalkboard, FaFilm, FaLightbulb, FaLongArrowAltRight, FaMagic, FaPenNib } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "graphics-animations");
    ...
    const steps = [
        {
          ...
          description: 'We begin by understanding your vision and brainstorming creative ideas to develop an engaging visual concept.',
          icon: <FaLightbulb className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Storyboarding',
          description: 'We create a detailed storyboard to outline the flow of the animation or design, ensuring clarity and cohesion.',
          icon: <FaChalkboard className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Design & Illustration',
          description: 'We bring your ideas to life through compelling graphic design and illustration that resonates with your audience.',
          icon: <FaPenNib className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Animation',
          description: 'We animate the design, adding motion and interactivity to create captivating and dynamic visual content.',
    ...
    return (
        <div className="about">
           <Head>
      <title>Graphics and Animations - Webroj Software Company</title>
      <meta name="description" content="Explore our stunning graphics and animations services that enhance user experience and engagement. We create visuals that captivate and communicate." />
    ...
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        <Link href="/" className="block text-lg font-bold sm:text-2xl lg:text-2xl whitespace-nowrap">
          Home/Graphics & Animations
        </Link>
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Graphics & Animations</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Graphics & <span className='text-[#fc005f]'>Animations</span>
  </h2>
