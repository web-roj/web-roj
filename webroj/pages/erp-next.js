...
import { FaCloudUploadAlt, FaHeadset, FaLongArrowAltRight, FaTools } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "erpnext");
    ...
    const steps = [
        {
          ...
          description: 'Tailored ERPNext implementation to streamline your business processes and improve operational efficiency.',
          icon: <FaTools className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Customization',
          description: 'Customizing ERPNext to meet your unique business requirements, ensuring a perfect fit for your workflows.',
          icon: <FaCogs className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Maintenance',
          description: 'Comprehensive support and maintenance services for ERPNext to ensure optimal performance and reliability.',
          icon: <FaHeadset className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Data Migration',
          description: 'Expert assistance in migrating your existing data to ERPNext with minimal disruption to your operations.',
    ...
    return (
        <div className="about">
          <Head>
      <title>ERPNext Solutions - Webroj Software Company</title>
      <meta name="description" content="Discover our ERPNext solutions designed to streamline your business operations, enhance productivity, and improve decision-making through real-time insights." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">ErpNext</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    ERPNext <span className='text-[#fc005f]'>Solutions</span>
  </h2>
