...
import { FaBug, FaLightbulb, FaLongArrowAltRight, FaMobileAlt, FaPaintBrush } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "android-ios");
    ...
    const steps = [
        {
          id: 1,
          title: 'App Ideation & Research',
          description: 'We begin by researching app ideas, understanding user needs, and analyzing market trends to shape the app’s concept.',
          icon: <FaLightbulb className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'UI/UX Design',
          description: 'We design engaging and user-friendly interfaces tailored for Android and iOS platforms, ensuring seamless navigation.',
          icon: <FaPaintBrush className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Development',
          description: 'We develop the app using Android (Java/Kotlin) or iOS (Swift) development environments, ensuring cross-platform compatibility.',
          icon: <FaMobileAlt className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Testing & QA',
          description: 'We test the app on various devices to ensure flawless performance, bug-free experience, and smooth functionality.',
          icon: <FaBug className="text-4xl text-pink-600" />
        },
    ...
    return (
        <div className="about">
          <Head>
      <title>Android & iOS Development - Webroj Software Company</title>
      <meta name="description" content="Explore our mobile app development services for Android and iOS platforms. We create high-quality, user-friendly applications that drive engagement and growth." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Android & Ios</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Mobile App  <span className='text-[#fc005f]'>Development</span>
  </h2>
