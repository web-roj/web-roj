...
import { FaBrain, FaCog, FaDatabase, FaLongArrowAltRight, FaRocket } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "aiml");
    ...
    const steps = [
        {
          ...
          description: 'We gather diverse data from multiple sources to train machine learning models and enhance decision-making capabilities.',
          icon: <FaDatabase className="text-4xl text-pink-600" />
        },
        {
          id: 2,
          title: 'Model Training',
          description: 'We train AI models using cutting-edge algorithms to accurately predict outcomes and solve complex problems.',
          icon: <FaBrain className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Optimization',
          description: 'We fine-tune models to ensure they perform efficiently and provide accurate insights across various scenarios.',
          icon: <FaCog className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Deployment',
          description: 'We deploy AI models in production, integrating them into real-world applications to deliver impactful solutions.',
  ...
  const [activeTab, setActiveTab] = useState('story');
    ...
    return (
       ...
       <>
       <Head>
      <title>AI & ML Services - Webroj Software Company</title>
      <meta name="description" content="Discover our AI and ML solutions that drive innovation and efficiency. Explore how our team can help you leverage advanced technologies for your business." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">About Us</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl">Empowering <span className='text-[#fc005f]'>AI Solutions</span></h2>
  <div className="flex flex-wrap justify-around gap-8 px-6">
    {steps.map((step) => (
      <div key={step.id} className="flex-1 min-w-[280px] border group hover:bg-gradient-to-r from-pink-500 to-red-500 bg-white p-8 rounded-lg shadow-lg relative">
        ...
        <h3 className="mb-4 text-2xl text-gray-800 group-hover:text-white">{step.title}</h3>
        <p className="text-justify text-gray-600 group-hover:text-white">{step.description}</p>
      </div>
    ))}
  </div>
</div>
