...
import { FaBug, FaCode, FaLightbulb, FaLongArrowAltRight, FaPencilRuler, FaServer } from 'react-icons/fa';
...
const About = () => {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
          try {
            ...
            const aboutUsBanners = bannersData.filter(banner => banner.category === "web-development");
    ...
    const steps = [
        {
          ...
          description: 'We analyze project requirements, research best practices, and create a roadmap for web development success.',
        ...
        
        {
          id: 2,
          title: 'Front-end',
          description: 'We build the user interface using modern web technologies like HTML, CSS, JavaScript, and frameworks like React.',
          icon: <FaCode className="text-4xl text-pink-600" />
        },
        {
          id: 3,
          title: 'Back-end ',
          description: 'We develop server-side logic and databases to ensure functionality, performance, and scalability.',
          icon: <FaServer className="text-4xl text-pink-600" />
        },
        {
          id: 4,
          title: 'Testing & Launch',
          description: 'We conduct thorough testing for performance, security, and functionality before deploying the website.',
    ...
    return (
        <div className="about">
           <Head>
      <title>Web Development Services - Webroj Software Company</title>
      <meta name="description" content="Discover our comprehensive web development services. We build responsive, user-friendly websites that enhance your online presence and drive business growth." />
  ...
  {banners.map(banner => (
    <div key={banner.id} className="relative w-full h-[150px] md:h-[200px]"> {/* Set a specific height for mobile */}
      ...
      <div className="absolute text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <span className="block mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl whitespace-nowrap">Web Development</span>
      </div>
    </div>
  ))}
</div>
  ...
  <h2 className="mb-8 text-2xl font-bold text-gray-800 lg:text-4xl md:text-4xl sm:text-3xl">
    Web App <span className='text-[#fc005f]'>Development</span>
  </h2>
